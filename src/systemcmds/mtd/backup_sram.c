 ...
 ****************************************************************************/
/* Configuration ************************************************************/
#  define CONFIG_RAMMTD_SIZE       32 // 32Kbits
#  define CONFIG_RAMMTD_BLOCKSIZE  32
#  define CONFIG_RAMMTD_ERASESIZE  32
          ...
          if (geo)
            {
              ...
              geo->neraseblocks = (CONFIG_RAMMTD_SIZE * 1024 / 8) / CONFIG_RAMMTD_BLOCKSIZE;
              ret               = OK;
          }
        }
        break;
...
FAR struct mtd_dev_s *rammtd_initialize(FAR uint8_t *start, size_t size)
{
  ...
  /* Following step 1~5 enable the backup SRAM feature */

  /* 1. Enable the power interface clock by setting the PWREN */
  modifyreg32(STM32_RCC_APB1ENR, 0, RCC_APB1ENR_PWREN);

  /* 2. Set the DBP bit to enable access to the backup domain */
  stm32_pwr_enablebkp();

  /* 3. Enable the RTC clock by programming the RTCEN */
  modifyreg32(STM32_RCC_AHB1ENR, 0, RCC_AHB1ENR_BKPSRAMEN);

  /* 4. Enable backup regulator */
  modifyreg32(STM32_PWR_CSR, 0, PWR_CSR_BRE);

  /* 5. Wait for backup regulator ready */
  while(!(getreg32(STM32_PWR_BASE + STM32_PWR_CSR_OFFSET) & PWR_CSR_BRR));
